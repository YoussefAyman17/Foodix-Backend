...
let forgetPassword =asyncHandler(async(req,res,next)=>{
    ...
    let hashedResetCode = crypto.createHash('sha256').update(resetCode).digest('hex');
...
let updatePassword =asyncHandler(async(req,res,next)=>{
    ...
    if(!currentPassword||!newPassword){
      return  next(new customError("You Must Provide Current And New Password",400))
    }
...
let deleteUserById =asyncHandler(async(req,res,next)=>{
    ...
    let user =await userModel.findByIdAndDelete(id);
      if(user){
        res.status(200).json({message:"User deleted Succesfully",DeletedId:id});
    }else{
        next(new customError("User Not Found",404))
    }
});
...
//
module.exports={deleteUserById,editUserById,getUserById,getAllUsers,login,signUP,updatePassword,forgetPassword,verifyResetCode,resetPassword}